...
// macro associate al Text del Msg
...
void Subqueue_init() {
    PoolAllocatorResult res = PoolAllocator_init(&_subqueues_allocator,
                                                SUBQUEUE_SIZE,
                                                MAX_NUM_PRIORITIES * MAX_NUM_RESOURCES, // MAX_NUM_PRIORITIES_RESOURCES
                                                _subqueues_buffer,
                                                SUBQUEUE_BUFFER_SIZE);

    if (res != Success) {
        printf("ERROR Failed to initialize message subqueues allocator!\n");
        assert(res == Success);
    }
}

// Resource* Resource alloc(int id, int type)
...
void MsgQueue_init() {
    ...
    if (res != Success) {
        printf("ERROR Failed to initialize message queues allocator!\n");
        assert(res == Success);
    }
}
